...
def unifica_dataset_merge_horario(diretorio_merge, lista_nome_dos_arquivos, variavel):
    """
    Agrega os arquivos do MERGE diário em apenas 1 dataset.

    Args:
      diretorio_merge (str): diretório onde se encontram os arquivos
      lista_nome_dos_arquivos (list): Lista de strings com o nome dos arquivos MERGE horários
      variavel (str): nome da variável desejada do dataset
    |
    Returns:
      xarray.Dataset: Dataset dos dados somados.
    |
    |
    """
    dataset = 0
    for arquivo_grib in lista_nome_dos_arquivos:
        dados = xr.open_dataset(f'{diretorio_saida}/{arquivo_grib}', engine='cfgrib')
        dataset = dataset + dados[variavel]
    return dataset